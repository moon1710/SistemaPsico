  ...
  try {
    ...
    if (psychologists.length > 0) {
      ...
      if (existing.length === 0) {
        ...
        console.log(
          `✅ Psicólogo ${psychologistId} asignado automáticamente al estudiante ${studentId}`
        );
      }
    } else {
      console.log(
        `⚠️ No hay psicólogos disponibles en la institución ${institucionId} para asignar al estudiante ${studentId}`
      );
    }
  } catch (error) {
  ...
  try {
    ...
    if (rol === "ESTUDIANTE" && institucionId) {
      try {
        ...
        console.error(
          "⚠️  Error en autoAssignPsychologist (no crítico):",
          error
        );
