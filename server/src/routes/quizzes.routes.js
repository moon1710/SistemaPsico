...
/**
 * QUIZ_ACCESS_MODE:
 *  - 'GLOBAL': cualquier usuario autenticado puede abrir quizzes ACTIVOS (ignora 'publico')
 *  - 'PUBLIC_FLAG': solo publico=1 y activo=1 (modo original)
 */
...
const ALERT_MIN_SEVERITY = "MODERADA";
...
async function findAssignedTutors(institucionId, alumnoId) {
  ...
  // B) fallback: ORIENTADOR/PSICOLOGO
  const [rows2] = await pool.execute(
    `SELECT ui.usuarioId AS tutorId, u.email,
...
async function safeInsertEmailOutbox(toUserId, toEmail, subject, body) {
  if (!toEmail) return;
  try {
    // email_outbox columns assumed:
    // (id AUTO or has default), to_user_id, to_email, subject, body, status, createdAt default
    await pool.execute(
      `INSERT INTO email_outbox (to_user_id, to_email, subject, body, status)
       VALUES (?, ?, ?, ?, 'PENDING')`,
      [toUserId || null, String(toEmail), String(subject), String(body)]
    );
  } catch (e) {
    // No rompemos el flujo si email_outbox tiene otra estructura.
    console.warn("[email_outbox] insert skipped:", e?.code || e?.message);
  }
}

/* -------------------- Notificar tutores + email -------------------- */
async function queueTutorNotifications({
  ...
  const notifBody =
    `Se registró un resultado ${severidad} (puntaje ${puntajeTotal}). ` +
    `Respuesta: ${attemptId}. Revise para canalizar.`;

  for (const t of tutors) {
    // App notification (tabla notificaciones, si existe con estas columnas)
    try {
      await pool.execute(
      ...
      // Si la tabla difiere, solo seguimos con email.
      console.warn("[notificaciones] insert skipped:", e?.code || e?.message);
    }

    // Email outbox (sin SMTP)
    await safeInsertEmailOutbox(
      t.tutorId,
      t.email,
      notifTitle,
      [
        `Hola ${t.tutorNombre || "Tutor"},`,
        ``,
        `Se registró un resultado ${severidad} para un estudiante asignado.`,
        `- Puntaje: ${puntajeTotal}`,
        `- Respuesta ID: ${attemptId}`,
        ``,
        `Inicie sesión para revisar detalles.`,
      ].join("\n")
    );
  }
}

/* -------------------- Crear cita desde alerta -------------------- */
async function createCitaRequestFromAlert({
  institucionId,
  alumnoId,
  severidad,
  respuestaId,
}) {
  if (!["MODERADA", "SEVERA"].includes(String(severidad))) return;
  try {
    await pool.execute(
      `INSERT INTO citas (id, institucionId, usuarioId, psicologoId, fechaHora, duracion, estado, source, respuestaId, severidad, fechaCreacion)
       VALUES (UUID(), ?, ?, NULL, NULL, 60, 'SOLICITADA', 'ALERTA', ?, ?, NOW(3))`,
      [
    ...
    console.warn("[citas from alerta] insert skipped:", e?.code || e?.message);
  }
}
...
async function ensureQuizAccessForUser(quizId, user) {
  ...
  if (QUIZ_ACCESS_MODE === "GLOBAL") {
    return quiz.activo === 1 ? quiz : null;
  }
...
// Resultados por institución (staff)
router.get(
  "/resultados",
  ...requireRolesWithInstitution([
    "PSICOLOGO",
  ...
  async (req, res) => {
    ...
    `;
    const dataSql = `
      SELECT rq.id, rq.fechaEnvio, rq.puntajeTotal, rq.severidad,
             q.id AS quizId, q.titulo, q.codigo, q.\`version\` AS version,
             u.id AS estudianteId, COALESCE(u.nombreCompleto, u.nombre) AS estudianteNombre
      ${baseFrom}
      ORDER BY rq.fechaEnvio DESC
  ...
  async (req, res) => {
    try {
      ...
      const [summary] = await pool.execute(
        `SELECT COUNT(*) AS total, AVG(rq.puntajeTotal) AS promedio,
                MIN(rq.puntajeTotal) AS minimo, MAX(rq.puntajeTotal) AS maximo,
...
router.post("/:quizId/submit", authenticateToken, async (req, res) => {
  try {
    ...
    // Notificar y crear cita (si alto)
    ...
    await createCitaRequestFromAlert({
      institucionId: instId,
      alumnoId: req.user.id,
      severidad,
      respuestaId: attemptId,
    });
