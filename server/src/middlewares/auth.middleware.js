...
const authenticateToken = async (req, res, next) => {
  try {
    ...
    // Verificar que la institución esté activa (excepto SUPER_ADMIN_NACIONAL)
    if (
...
const requireInstitutionAccess = (req, res, next) => {
  ...
  // SUPER_ADMIN_NACIONAL tiene acceso global
  if (req.user.rol === "SUPER_ADMIN_NACIONAL") {
    return next();
  }

  // Otros roles solo pueden acceder a su institución
  if (req.user.institucionId !== parseInt(institucionId)) {
    return res.status(403).json({
      success: false,
...
/**
 * Middleware solo para SUPER_ADMIN_NACIONAL
 */
...
/**
 * Middleware para admins de institución (SUPER_ADMIN_INSTITUCION + SUPER_ADMIN_NACIONAL)
 */
...
/**
 * Middleware para psicólogos (incluyendo admins)
 */
